...
public class DBAppointment {
    ...
    public static boolean addAppointment(String type, int userId, int custId, ZonedDateTime start, ZonedDateTime end){
        if(getAptsByUserTime(start, end, userId).isEmpty()){
            try{
                String statement = "INSERT INTO appointment VALUES(null,?,?,?,?,?,?,?,?,?,?,?,?,null,?)";
                PreparedStatement ps = DBConnection.getConnection().prepareStatement(statement);

            //key mapping
                ps.setInt(1, custId);
                ps.setInt(2, userId);
                ps.setString(3, "not applicable");
                ps.setString(4, "not applicable");
                ps.setString(5, "not applicable");
                ps.setString(6, "not applicable");
                ps.setString(7, type);
                ps.setString(8, "not applicable"); 
                ps.setTimestamp(9, toUTCTimestamp(start));
                ps.setTimestamp(10, toUTCTimestamp(end)); 
                ps.setTimestamp(11, toUTCTimestamp(ZonedDateTime.now()));
